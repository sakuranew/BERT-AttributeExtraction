...
def strip(filename):
    ...
    with open(filename, 'r', encoding='utf-8') as f:
        ...
        while True:
            ...
            # _sentence=" ".join(_sentence)
            f_sentence=""
            for i,ss in enumerate(_sentence,start=s):
                f_sentence+=ss
                if i<len(all_symbol) and i<e:
                    f_sentence+=all_symbol[i]

            content[3]=f_sentence
            new_content="#".join(content)
            w.write(new_content)
            w.write('\n')
            if len(f_sentence)>maxl:
                maxl=len(f_sentence)
        print((maxl))
if __name__=="__main__":
    file_list=["./data/birth_date_train.txt","./data/birth_date_test.txt"]
    for f in file_list:
        strip(f)